// src/routes/cards.js
// Card management: list, deploy, retire, upgrade, merge, sell
...
router.post('/buy', (req, res) => {
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, 1, 0, 100, ?, ?, ?)
    `).run(cardId, uid, type.rarity, type.name, type.base_mushrooms, type.base_mushrooms, type.color);
...
router.post('/:id/upgrade', (req, res) => {
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      UPDATE cards SET level = ?, xp = 0, xp_needed = ?, mushrooms_per_cycle = ?
      WHERE id = ? AND user_id = ?
    `).run(newLevel, newLevel * 100, newMpc, card.id, uid);
...
router.post('/merge', (req, res) => {
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      UPDATE cards SET level = ?, xp = 0, xp_needed = ?, mushrooms_per_cycle = ?
      WHERE id = ?
    `).run(newLevel, newLevel * 100, newMpc, base.id);
  })();
...
router.post('/:id/deploy', (req, res) => {
  ...
  if (existingSlot && existingSlot.card_id) {
    return res.status(409).json({ error: 'Slot déjà occupé' });
  }
  ...
  db.transaction(() => {
    ...
    db.prepare(`
      INSERT INTO farm_slots (user_id, slot_index, card_id, farming_since)
      VALUES (?,?,?,?)
